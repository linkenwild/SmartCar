/*http://www.openedv.com/forum.php?mod=viewthread&tid=79541&highlight=mpu6050*/
...
void Angle_Calcu(short *accel, short *gyro, float *angle_temp, float *angle_final)
{
	...
	Accel_x = accel[0]; //x轴加速度值暂存
	Accel_y = accel[1]; //y轴加速度值暂存
	Accel_z = accel[2]; //z轴加速度值暂存
	Gyro_x = gyro[0];  //x轴陀螺仪值暂存
	Gyro_y = gyro[1];  //y轴陀螺仪值暂存
	Gyro_z = gyro[2];  //z轴陀螺仪值暂存
...
//	Erjielvbo(Angle_x_temp, Gyro_x);
	...
	Kalman_Filter_Z(Angle_z_temp, Gyro_z);  //卡尔曼滤波计算Y倾角
        ...
        for(int i=0; i<3;i++)
        {
          angle_temp[0] = Angle_x_temp;
          angle_temp[1] = Angle_y_temp;
          angle_temp[2] = Angle_z_temp;
        }
          
         for(int i=0; i<3;i++)
        {
          angle_final[0] = Angle_X_Final;
          angle_final[1] = Angle_Y_Final;
          angle_final[2] = Angle_Z_Final;
        }
