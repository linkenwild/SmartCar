...
static signed char gyro_orientation[9] = { 1, 0, 0,
                                           0, 1, 0,
                                         0, 0, 1};
#if MPU6050_OFFSET_CALC
long a_offset[3] = {0}, g_offset[3] = {0};
#endif
...
uint8_t run_self_test(void)
{
	...
	if (result == 0x3) 
	{
		...
		*/
		...
		#if MPU6050_OFFSET_CALC
			memcpy(a_offset, accel, sizeof(accel));
			memcpy(g_offset, gyro, sizeof(gyro));
		#endif
...
//返回值:0,正常
//    其他,失败
uint8_t mpu_dmp_get_data(float *pitch, short* gyro_y,short* accel_y )
{
	...
	**/
	if(sensors&INV_WXYZ_QUAT) 
	{
		...
		*pitch = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3;	// pitch
		...
		//*yaw   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw   无修正，不启用
                *gyro_y = gyro[1];
                *accel_y = accel[1];
